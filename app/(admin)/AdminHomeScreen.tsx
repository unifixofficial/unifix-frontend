    ...
    <View style={styles.root}>
     ...
     <View>
        ...
        <TouchableOpacity style={styles.bellBtn} activeOpacity={0.7}>
          <Ionicons name="notifications-outline" size={22} color="#1a3c2e" />
        </TouchableOpacity>
      </View>
...
{pendingStaff.length > 0 && (
  <TouchableOpacity
    style={styles.pendingStaffCard}
    activeOpacity={0.8}
    onPress={() => router.push("/(admin)/MaintenanceScreen" as any)}
  >
    ...
    <Ionicons name="chevron-forward" size={18} color="#64748b" />
        ...
        <View style={styles.mgmtSection}>
          <Text style={styles.mgmtTitle}>Management</Text>
          <View style={styles.mgmtGrid}>
            {[
              { label: "Maintenance", icon: "construct", color: "#3b82f6", bg: "#eff6ff", route: "/(admin)/MaintenanceScreen" },
              { label: "Staff & Users", icon: "people", color: "#8b5cf6", bg: "#f5f3ff", route: "/(admin)/StaffUsersScreen" },
              { label: "ID Cards", icon: "card", color: "#f59e0b", bg: "#fffbeb", route: "/(admin)/IdCardsScreen" },
              { label: "Deletions", icon: "trash", color: "#ef4444", bg: "#fef2f2", route: "/(admin)/DeletionsScreen" },
              { label: "Security", icon: "shield-checkmark", color: "#16a34a", bg: "#f0fdf4", route: "/(admin)/SecurityScreen" },
            ].map((item) => (
              <TouchableOpacity
                key={item.label}
                style={styles.mgmtCard}
                activeOpacity={0.82}
                onPress={() => router.push({ pathname: item.route as any, params: {} })}
              >
                <View style={[styles.mgmtIconWrap, { backgroundColor: item.bg }]}>
...
const styles = StyleSheet.create({
 ...
 appSubtitle: { fontSize: 11, color: "#4ade80", fontWeight: "600" },
  bellBtn: { width: 38, height: 38, borderRadius: 19, backgroundColor: "#ffffff", alignItems: "center", justifyContent: "center", shadowColor: "#000", shadowOpacity: 0.05, shadowRadius: 6, elevation: 2 },
  scroll: { flex: 1 },
 ...
 mgmtSection: { paddingHorizontal: 20, marginBottom: 20 },
  mgmtTitle: { fontSize: 17, fontWeight: "800", color: "#1a3c2e", marginBottom: 14 },
  mgmtGrid: { flexDirection: "row", flexWrap: "wrap", gap: 12 },
  mgmtCard: {
    width: (SCREEN_WIDTH - 40 - 12 * 2) / 3,
    backgroundColor: "#ffffff",
    borderRadius: 16,
    padding: 14,
    alignItems: "center",
  ...
  mgmtIconWrap: { width: 48, height: 48, borderRadius: 14, alignItems: "center", justifyContent: "center" },
  mgmtLabel: { fontSize: 11, fontWeight: "700", color: "#1a3c2e", textAlign: "center" },
  fab: {
    position: "absolute",
    bottom: 90,
    right: 20,
