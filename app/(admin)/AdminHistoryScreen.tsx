...
function HistoryCardSkeleton({ color = "#ef4444" }: { color?: string }) {
  return (
    <View style={[styles.flaggedCard, { opacity: 0.6, borderLeftColor: color }]}>
      <View style={{ flexDirection: "row", justifyContent: "space-between", marginBottom: 10 }}>
        <View style={{ height: 20, width: 80, backgroundColor: "#f1f5f9", borderRadius: 6 }} />
        <View style={{ height: 16, width: 16, backgroundColor: "#f1f5f9", borderRadius: 8 }} />
      </View>
      <View style={{ height: 16, width: "75%", backgroundColor: "#f1f5f9", borderRadius: 6, marginBottom: 8 }} />
      <View style={{ height: 13, width: "50%", backgroundColor: "#f1f5f9", borderRadius: 6, marginBottom: 8 }} />
      <View style={{ height: 13, width: "40%", backgroundColor: "#f1f5f9", borderRadius: 6, marginBottom: 8 }} />
...
export default memo(function AdminHistoryScreen({
 ...
 const completedComplaints = useMemo(() => allComplaints.filter((c) => c.status === "completed" && (c.flagged === true || c.flagged === 1)), [allComplaints]);
const flaggedActive = useMemo(() => allComplaints.filter((c) => (c.flagged === true || c.flagged === 1) && !c.flagResolved && c.status !== "completed"), [allComplaints]);
const flaggedHistory = useMemo(() => allComplaints.filter((c) => (c.flagged === true || c.flagged === 1) && (c.flagResolved === true || c.flagResolved === 1) && c.status !== "completed"), [allComplaints]);
    ...
    <View style={styles.root}>
      ...
      >
        ...
        {activeTab === "completed" && (
          ...
          {refreshing ? (
              <>
                <HistoryCardSkeleton color="#16a34a" />
                <HistoryCardSkeleton color="#16a34a" />
                <HistoryCardSkeleton color="#16a34a" />
              </>
