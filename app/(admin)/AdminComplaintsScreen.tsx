...
function ComplaintCardSkeleton() {
  return (
    <View style={[styles.complaintCard, { opacity: 0.6 }]}>
      <View style={{ flexDirection: "row", justifyContent: "space-between", marginBottom: 8 }}>
        <View style={{ height: 12, width: 120, backgroundColor: "#f1f5f9", borderRadius: 6 }} />
        <View style={{ height: 20, width: 70, backgroundColor: "#f1f5f9", borderRadius: 8 }} />
      </View>
      <View style={{ height: 16, width: "70%", backgroundColor: "#f1f5f9", borderRadius: 6, marginBottom: 10 }} />
      <View style={{ flexDirection: "row", gap: 8, marginBottom: 8 }}>
        <View style={{ height: 12, width: 100, backgroundColor: "#f1f5f9", borderRadius: 6 }} />
        <View style={{ height: 12, width: 80, backgroundColor: "#f1f5f9", borderRadius: 6 }} />
      </View>
      <View style={{ height: 36, backgroundColor: "#f1f5f9", borderRadius: 8 }} />
    </View>
  );
}
...
const isFlagged = (complaint.flagged === true || complaint.flagged === 1) && !complaint.flagResolved && ["pending", "assigned", "in_progress"].includes(complaint.status);
const wasFlagged = complaint.flagged === true || complaint.flagged === 1;
    ...
    <Modal visible={visible} animationType="slide" onRequestClose={onClose}>
      <GestureHandlerRootView style={{ flex: 1 }}>
        ...
        {complaint.flagged && !complaint.flagResolved && complaint.status !== "completed" && complaint.status !== "rejected" && (
              ...
              <TouchableOpacity style={styles.iwillhandleBtn}onPress={() => { onIWillHandle(complaint.id); }} disabled={actionLoading === "iwillhandle_" + complaint.id}>
                    {actionLoading === "iwillhandle_" + complaint.id ? (
              ...
              <TouchableOpacity style={styles.resolveBtn} onPress={() => { onMarkResolved(complaint.id); }} disabled={actionLoading === "resolve_" + complaint.id}>
                  {actionLoading === "resolve_" + complaint.id ? (
                    <ActivityIndicator size="small" color="#ffffff" />
                  ) : (
                    <>
